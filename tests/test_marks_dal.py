...
def test_marks_aggregates_and_weights(db_conn, db_tx):
    ...
    mod_id = modules_dal.create_module(user_id=uid, module_code="COS2626",
                                       ...
                                       min_assignments=1)
    a1 = assignments_dal.create_assignment(user_id=uid, module_id=mod_id, assignment_title="FA1", category="Formative", assignment_type="Quiz", due_date=dt.date.today(), submit_status="Done")
    a2 = assignments_dal.create_assignment(user_id=uid, module_id=mod_id, assignment_title="FA2", category="Formative", assignment_type="Practical", due_date=dt.date.today(), submit_status="Done")
    a3 = assignments_dal.create_assignment(user_id=uid, module_id=mod_id, assignment_title="EX1", category="Exam", assignment_type="Written exam", due_date=dt.date.today(), submit_status="Done")
